...
function renderStockPage(property){
	...
	$('#export-stock-form-container').html(`
		...
		<label class="mb-0" for="input-export-stock-select-item">${Utils.translate('stock_page_modal_label_item')}</label>
	...
	$('#input-export-stock-select-vehicle').append(`<option value="" selected disabled>${Utils.translate('stock_page_modal_placeholder_vehicle')}</option>`);
	for (const vehicle of factory_vehicles) {
		$('#input-export-stock-select-vehicle').append(`<option vehicle_id="${vehicle.id}" trunk="${config.vehicles[vehicle.vehicle].trunk}">${config.vehicles[vehicle.vehicle].name} (${config.vehicles[vehicle.vehicle].trunk} ${Utils.translate('weight_unit')})</option>`);
	}
	if (item.data.last_vehicle && item.data.last_vehicle.network_id) {
		$('#input-export-stock-select-vehicle').append(`<option vehicle_id="spawned_from_world" trunk="${item.data.last_vehicle.vehicle_data.trunk}">${item.data.last_vehicle.vehicle_data.name} (${item.data.last_vehicle.vehicle_data.trunk} ${Utils.translate('weight_unit')})</option>`);
	}
