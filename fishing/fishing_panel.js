$(document).ready(function () {
    ...
    async function cancelReset(message, pass) {
      ...
      setTimeout(function () {
        ...
        if (pass == false) {
          $.post(`https://NW_fishing/close`, JSON.stringify({ success: pass }));
          progress = 0;
          animCancel = false;
          bobbing = false;
          nibble = false;
        ...
        if (add === true) {
          if (width >= tension || reeling == false || fishOn == false) {
            ...
            if (width >= tension) {
              ...
              $.post(
                `https://NW_fishing/updateTrackingFish`,
                JSON.stringify({ progress: progress, isitover: true })
              );
              return;
            }
          } else {
